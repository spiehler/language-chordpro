...
import ChordproModel from './ChordproModel';
...
export default class LanguageChordproView {
  ...
  constructor(serializedState) {
    ...
    this.chordproModel = new ChordproModel();

    this.subscriptions = atom.workspace.getCenter().observeActivePaneItem(item => {

      // clear array elements
      this.chordproModel.clearChords();
      // clear content
  ...
  parseScore(editor) {
    ...
    editor.scan(new RegExp('^{(k|key):[A-Zmb# ]*}', 'g'), iterator => {
      ...
      this.chordproModel.setScoreKey(scoreKey[0].toString());
    });
    // scan for chords
    editor.scan(new RegExp('\\[[a-zA-Z0-9#\\/]*\\]', 'g'), iterator => {
      this.chordproModel.addChord(iterator.match[0].slice(1, iterator.match[0].length-1));
    });
  }
  ...
  showChordPanel() {
    ...
    scoreKey.innerHTML = this.chordproModel.getScoreKey();
    ...
    for (let i = 0; i < this.chordproModel.getKeys().length; i++) {
      let key = document.createElement('option');
      key.innerHTML = this.chordproModel.getKey(i);
      keysBox.appendChild(key);
    }
    keysBox.value = this.chordproModel.getScoreKey();
    ...
    for (let i = 0; i < this.chordproModel.getSteps().length; i++) {
      let step = document.createElement('option');
      step.innerHTML = this.chordproModel.getStep(i);
    ...
    transposeButton.addEventListener('click', this.transposeSong.bind(this), false);
    ...
    // add chord buttons
    for (let i = 0; i < this.chordproModel.getChords().length; i++) {
      let chordButton = document.createElement('button');
      chordButton.classList.add('btn');
      chordButton.innerHTML = this.chordproModel.getChord(i);
  ...
  transposeSong(event) {
    ...
    let newChord = this.chordproModel.transposeChord('C', -2);
