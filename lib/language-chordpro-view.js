...
export default class LanguageChordproView {
  ...
  parseScore(editor) {
    ...
    editor.scan(new RegExp('^{(k|key):[A-Zmb# ]*}', 'g'), iterator => {
      this.chordModel.setScoreKey(iterator.match[0].match(new RegExp('[A-Z][mb#]?')));
    });
    // scan for chords
  ...
  showChordPanel() {
    ...
    for (let i = 0; i < this.chordModel.getChords().length; i++) {
      ...
      //chordButton.onclick = this.insertChord(chordButton.innerHTML);
      chordButton.addEventListener('click', this.insertChord(chordButton.innerHTML));
